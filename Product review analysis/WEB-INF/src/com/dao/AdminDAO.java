...
public class AdminDAO
{
	...
	public static  boolean addReviewDetails(String product,RichTextString richTextString)
	{
		boolean flag=false;
		String sql = "";
		int i=0;
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			sql = "insert into m_review(r_product,reviews) values('"+product+"','"+richTextString+"')";
...
/* Get Specific Server Details (Starts) */
	...
	public static ResultSet getProductSpecificDetails(int proId)
	{
		String sql="";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			sql="select * from m_product where pro_code='"+proId+"'";
			System.out.println("******* Get Specific Server Info *********\n");
			System.out.println(sql);
			resultSet = statement.executeQuery(sql);
		}
...
/* Add User Details (Starts) */
	...
	public static boolean addProductDetails(String pname,int locde,String paddr,String pcontact,String seller) 
	{
		boolean flag=false;
		...
		try
		{
			...
			connection=dao.connector();
			Statement stmt = connection.createStatement();
			//insert into m_mal (m_name,m_address,m_contact,c_code)values ('dfdf','dfdf','dfd','dfdf')
			String sql="insert into m_product (pro_name,cat_code,pro_description,pro_price,pro_seller_details)values ('"+pname+"','"+locde+"','"+paddr+"','"+pcontact+"','"+seller+"')";
...
/* Get Department By Id(Starts) */
	...
	public String getProductDetails1(int id)
	{
		String name="";
		String sql = "";
		try
...
/* Get Department By Id(Ends) */
	...
	public boolean checkNoun(String nounword) 
	{
		boolean flag=false;
		String sql = "";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			sql = "select * from m_noun where noun_word='"+nounword.trim()+"'";
			System.out.println("******* Check User Existence *********\n");
			System.out.println(sql);
			resultSet = statement.executeQuery(sql);
			while(resultSet.next())
	...
	public boolean checkAdj(String adjword) 
	{
		boolean flag=false;
		String sql = "";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			sql = "select * from m_adjective where  adj_word='"+adjword.trim()+"'";
			System.out.println("******* Check User Existence *********\n");
			System.out.println(sql);
			resultSet = statement.executeQuery(sql);
			while(resultSet.next())
	...
	public static boolean check_noun(String wor)
	{
		boolean flg = false;
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
		String sql = "select *from m_noun where noun_word='"+wor+"'";
	...
	public static boolean check_adjective(String wor)
	{
		boolean flg = false;
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
		String sql = "select * from m_adjective where adj_word='"+wor+"'";
	...
	public boolean add(String ufileName,String ufileType, String udate,String subject,int cloudId,int ownerId) 
	{
		boolean flag=false;
		String sql="";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			sql="insert into m_file_upload (f_name,f_type,f_upload_date,f_subject,cloud_id,owner_id,flag) values('"+ufileName+"','"+ufileType+"','"+udate+"','"+subject+"','"+cloudId+"','"+ownerId+"','false')";
			System.out.println(sql);
			int i=statement.executeUpdate(sql);
			if(i!=0)
			{
...
/* Get Department Details (Starts) */
	...
	public static String getCategory1(int no)
	{
		String name="";
		ResultSet rs=null;
		String sql="";
		try
		{
			...
			connection=dao.connector();
			Statement stmt= connection.createStatement();
			sql="select cat_name from m_category where cat_id='"+no+"'";
			System.out.println("******* Domain Info *********\n");
			System.out.println(sql);
			rs = stmt.executeQuery(sql);
	...
	public static ResultSet getOTOW()
	{
		String sql="";
		try
		{
...
/* Get Upload File Details(Ends) */
	...
	public ResultSet getUploadedFiles(int ownerId)
	{
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			String sql="select * from m_file_upload where owner_id='"+ownerId+"' and flag='false'";
			resultSet = statement.executeQuery(sql);
			System.out.println("Access Control---->"+sql);
		}
		catch(Exception e)
		{
			System.out.println("Opp's Error is in AdminDAO-getUploadedFiles(int ownerId) : ");
...
/* Get Upload File Details By fileId(Starts) */
	...
	public String getUploadFiles(int fileId)
	{
		...
		try
		{
			...
			resultSet = statement.executeQuery(sql);
			System.out.println("@@@@@@@@@@@ Upload Info @@@@@@@@@@@@@@@"+sql);
			while(resultSet.next())
			{
...
/* Add AcessControl Details (Starts) */
	...
	public static boolean changeFlag(int fileCode) 
	{
		boolean flag=false;
		String sql = "";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			String sql1="update m_file_upload set flag='true' where f_code='"+fileCode+"'";
			int i=statement.executeUpdate(sql1);
			System.out.println("Flag querryy-->>"+flag);
			if(i!=0)
			{
				flag=true;
			}
			System.out.println("Flag value in File Upload changed : "+flag);
		
		}
		catch(Exception e)
		{
			System.out.println("Opp's Error is in AdminDAO-change Flag :" );
			e.printStackTrace();
		}
		return flag;
	}
	
	public static boolean requestChangeFlag(String userid) 
	{
		boolean flag=false;
		String sql = "";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			String sql1="update m_request_secfile set flag='true' where userid='"+userid+"'";
			int i=statement.executeUpdate(sql1);
			System.out.println("Flag querryy-->>"+sql1);
			if(i!=0)
			{
				flag=true;
			}
			System.out.println("Flag value in request Secret key: "+flag);
		
		}
		catch(Exception e)
		{
			System.out.println("Opp's Error is in AdminDAO-requestChangeFlag :" );
			e.printStackTrace();
		}
		return flag;
	}
...
public static ResultSet getfiles()
{
	...
	try
	{
		...
		connection=dao.connector();
		statement = connection.createStatement();
		resultSet = statement.executeQuery("select * from m_transaction");
...
public static boolean checkAlreadyExist(String userid)
{
	...
	try
	{
		...
		connection=dao.connector();
		statement = connection.createStatement();
		resultSet = statement.executeQuery("select * from m_product where pro_code='"+userid+"'");
		while(resultSet.next())
		{
			flag=true;
		}
...
public static boolean addUser(String procode, String proname, String catcode,
		String prodesc, String proprice,String proseller){
		boolean flag=false;
		try
		{
			DAO dao=DAO.getInstance();
			...
			String sql="insert into m_product(pro_name,cat_code,pro_description,pro_price,pro_seller_details) values('"+proname+"','"+catcode+"','"+prodesc+"','"+proprice+"','"+proseller+"')";
   ...
   public static boolean AddReview(String productid,String review) {
	boolean flag=false;
	try
	{
		DAO dao=DAO.getInstance();
		...
		String sql="insert into m_review(pro_code,review_comment)values('"+productid+"','"+review+"')";
